...
public class UserExitThread {
    ...
    private static class MessageLoop implements Runnable {
        public void run() {
            ...
            for(String important: importantInfo){
                try {
                    Thread.sleep(3000);
    ...
    public static void main(String args[]) throws InterruptedException {
        ...
        threadMessage("Press <enter> to exit:");
        try(Scanner scanner = new Scanner(System.in)) {
            scanner.nextLine();
        }
        threadMessage("Got input. Closing thread...");
        if (t.isAlive()) {
            t.interrupt();
            t.join(5000);
            if(t.isAlive())
...
//                t.interrupt();
//                // Shouldn't be long now -- wait indefinitely (dangerous: never do this!)
//                t.join();
//            }
//        }
